...
function handleProductFormSubmit(e){
  ...
  try {
    ...
    const saved = GmpesStore.upsertProduct(product);
    if (!saved){
      errorEl.textContent = "Não foi possível salvar. Verifique se o navegador permite armazenamento local (localStorage) e tente novamente.";
      return false;
    }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!GmpesStore.isStorageAvailable()){
    document.body.insertAdjacentHTML("afterbegin", `
      <div style="background:#4a1010; color:#ffcccc; padding:14px 20px; text-align:center; font-size:13.5px; font-family:sans-serif; position:relative; z-index:999;">
        Este navegador está bloqueando o armazenamento local (localStorage), então não será possível salvar patches.
        Isso costuma acontecer em modo anônimo/privado com bloqueio de cookies, ou ao abrir o arquivo diretamente
        (file://) em vez de acessá-lo por um servidor/domínio. Tente em uma aba normal ou verifique as
